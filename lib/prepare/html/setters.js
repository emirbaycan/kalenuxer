...
function setVal(file_text, data) {
    ...
    for (match in matches) {
        ...
        // Skip if this is part of a Go template (double braces {{variable}})
        // Check if the match is preceded or followed by another brace
        const matchIndex = file_text.indexOf(match);
        if (matchIndex > 0 && file_text[matchIndex - 1] === '{') {
            continue; // This is {{variable}}, skip it
        }
        if (matchIndex + match.length < file_text.length && file_text[matchIndex + match.length] === '}') {
            continue; // This is {variable}}, skip it
        }
