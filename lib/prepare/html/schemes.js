...
export function updateScheme(file_type) {
    ...
    function setScheme(file_path) {
        ...
        if(global.icons_option == false && file_path == 'css/users/general/icons.css'){
            return;
        }
        file_path = file_path.split(file_type + '/')[1];
        data_path = file_path.split('/');
        if (file_type === 'html' || file_type === 'php' || file_type === 'go') {
            file_path = file_path.replace('users/', '/').replace('main/products', '').replace('main/pages', '').replace('main/templates', '').replace('.kalenux.html', '').replace('.kalenux.php', '.php').replace('.kalenux.go', '.go');
        } else {
