...
function addCSSs(file_content, data_mem) {
    ...
    for (var i = 0; i < data_mem.length; i++) {
        ...
        // Aggressive filtering to prevent invalid entries
        if (!mem || !mem.path || typeof mem.path !== 'string' || mem.path.length <= 1) {
            continue;
        }
        
        // Additional check for proper file paths
        if (!mem.path.includes('.') || mem.path.startsWith('.') || mem.path.indexOf('/') === -1) {
            continue;
        }
        ...
        // Final safety check - if path is suspiciously short, skip it
        if (path.length <= 4) { // "/x" would be length 4
            continue;
        }
        ...
        type = mem.type;
        // Handle type as array (from split result)
        const typeStr = Array.isArray(type) ? type.join('.') : type;
        
        // Only process if we have a valid path
        if (!mem.path || mem.path.length <= 3 || !mem.path.includes('.') || mem.path.startsWith('.') || mem.path.indexOf('/') === -1) {
            continue;
        }
...
export function htmlJSCSS(file_content, data_path, language) {
    ...
    // Convert string data_path to array to prevent character iteration
    if (typeof data_path === 'string') {
        data_path = data_path.split('/').filter(part => part.length > 0);
    }
    
    // If data_path is still not a valid array, return unchanged content
    if (!Array.isArray(data_path) || data_path.length === 0) {
        return file_content;
    }
    ...
    for (asset in assets) {
        ...
        if (rem_datas.general) {
            ...
            for (smem in mem) {
                smem = mem[smem];
                const processedPath = smem.replace('.lang', '.' + language);
                if (processedPath && processedPath.length > 3 && processedPath.includes('.') && !processedPath.startsWith('.') && processedPath.indexOf('/') !== -1) {
                    data.push({ type: smem.split('.').slice(1, -1), path: processedPath, degree: -1 })
                }
            }
        }

        while (path = rem_data_path[0]) {
            // Comprehensive check to prevent string iteration
            if (typeof rem_data_path === 'string' || 
                typeof path !== 'string' ||
                path.length === 1 ||
                degree > 10) { // Safety limit to prevent infinite loops
                break;
            }
            if (path.indexOf('.') !== -1) {
                path = path.split('.')[0];
                ...
                if (mem_data && mem_data[path]) {
                    const foundData = mem_data[path];
                    if(typeof foundData !== 'string'){
                        // It's an object, continue navigating deeper
                        break;
                    }
                    // It's a string (file path), process it
                    const processedPath = foundData.replace('.lang', '.' + language);
                    // Skip invalid paths
                    if (processedPath && processedPath.length > 3 && processedPath.includes('.') && !processedPath.startsWith('.') && processedPath.indexOf('/') !== -1) {
                        data.push({ type: foundData.split('.').slice(1, -1), path: processedPath, degree: degree });
                    }
                }
                break;
            }
            ...
            if (!mem_data) {
                ...
                for (smem in mem) {
                    ...
                    // Skip invalid paths
                    if (processedPath && processedPath.length > 3 && processedPath.includes('.') && !processedPath.startsWith('.') && processedPath.indexOf('/') !== -1) {
                        data.push({ type: smem.split('.').slice(1, -1), path: processedPath, degree: degree });
                    }
                }
            }
            degree += 1;
