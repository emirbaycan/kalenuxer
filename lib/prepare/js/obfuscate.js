...
<<<<<<< HEAD
    matches = file_text.match(/\.classList\.(add|remove|contains)\("[\w \-]+"\)/g);
=======
...
<<<<<<< HEAD
    matches = file_text.match(/\.id="[_\w \-]+"/g);
=======
...
<<<<<<< HEAD
        file_text = file_text.replace(new RegExp('\\.classList\\.(add|remove|contains)\\("' + property + '"\\)', 'g'), '.classList.$1("' + replacement + '")');
=======
