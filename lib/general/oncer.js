...
export function onceTemplateProcessed(time, file_name, file_path) {
    ...
    var html_time = 'html';
    var html_times = global.times[html_time];
    var html_times_keys = Object.keys(html_times);

    for(var key in html_times_keys){
        var html_key = html_times_keys[key];
        if(html_key.indexOf(directory) !== -1){
            if(!global.times[html_time][html_key]){
                global.times[html_time][html_key] = {};
            }
            global.times[html_time][html_key].processed_at = file_time;
        }
    }

    updateTimes(html_time);
    updateTimes(time);
    return true;
}
