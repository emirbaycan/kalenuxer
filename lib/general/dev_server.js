...
export async function startDevServer({ projectRoot, port = 3000 }) {
    ...
    app.use((req, res, next) => {
        if (/\.(css|js|png|jpeg|jpg|json|webp|ttf|mp4)$/.test(req.url)) {
    ...
    // 2. **CORS Headers**
    app.use((req, res, next) => {
        res.setHeader("Access-Control-Allow-Origin", "https://karalarbigbag.com");
        res.setHeader("Access-Control-Allow-Credentials", "true");
        res.setHeader("Access-Control-Allow-Headers", "Origin, Content-Type");
        res.setHeader("Strict-Transport-Security", "max-age=1; includeSubDomains; preload");
        next();
    });
    ...
    chokidar.watch(watchDirs, { ignoreInitial: true }).on("all", async (event, filePath) => {
        console.log(`[DevServer] ${event}: ${filePath}. Rebuilding...`);
        await processKalenuxer({ projectRoot, outputDir });
